...
class AppDelegate: NSObject, NSApplicationDelegate
{
	...
	func applicationDidFinishLaunching (_ aNotification: Notification)
	{
		...
		// managedObjectContext environment keyPath.
		// Add `@Environment(\.managedObjectContext)` in the views that will
		// need the context.
		let contentView = ToolsView().environment(
	...
	lazy var persistentContainer: NSPersistentContainer =
	{
		...
		// store to fail.
		let container = NSPersistentContainer(name: "Sildish_MacOS")
	...
	@IBAction func saveAction(_ sender: AnyObject?) {
		...
		{
			NSLog(
				"\(NSStringFromClass(type(of: self))) unable to commit "
	...
	) -> NSApplication.TerminateReply
	{
		...
		catch
		{
			...
			let info = NSLocalizedString(
				"Quitting now will lose any changes you have made since the "
